...
A list of supported algorithms depends on the versions of Logstash, JRuby, and Java this plugin is running in, but can be obtained by
[source,ruby]
...
Vector (IV) per encryption operation, a standard best-practice to ensure
...
re-used up to `max_cipher_reuse` times before re-created
...
===== `mode` 
  ...
  * This is a required setting.
  * Value type is <<string,string>>
  ** `encrypt`: perform encryption of a plaintext value into IV + ciphertext
  ** `decrypt`: perform decryption of an IV + ciphertext value into plaintext
  * There is no default value for this setting.
